...
// RenderPage 渲染页面
func RenderPage(path string, ctx *gin.Context) {
	mod, has := model.PostSingle(path)
	if !has {
		ctx.Redirect(302, "/")
		return
	}
	mod.Richtext = regImg.ReplaceAllString(mod.Richtext,
		`<img class="lazy-load" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="$1" alt="$2">`)
	ctx.HTML(http.StatusOK, "page", map[string]interface{}{
		"Page":   mod,
		"Show":   mod.Status == 2,
		"global": model.Gcfg(),
