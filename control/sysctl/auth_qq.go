...
func ViewLoginQq(ctx *gin.Context) {

	ctx.Redirect(302, fmt.Sprintf("https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=%s&redirect_uri=%s&state=zxysilent&", conf.App.Qq.WebAppid, redirectUrl)) //临时
}

func ViewAuthQq(ctx *gin.Context) {
	loginUrl := fmt.Sprintf("https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=%s&client_secret=%s&code=%s&redirect_uri=%s&fmt=json", conf.App.Qq.WebAppid, conf.App.Qq.WebSecret, ctx.GetString("code"), redirectUrl)
	atoken := struct {
